const isValid = function (x) {
    ...
    else if (typeof x === 'string' && x.trim().length === 0) return false
    else return true
}
...
const isValidMobile = function (y) {
    let mobileRegex = /^(?:(?:\+|0{0,2})91(\s*\s*)?|?)?[6789]\d{9}$/
...
const isValidGender = function (gender) {
    let orderStatus = ["Male", "Female", "Other"]
    return orderStatus.includes(gender)
}

module.exports = { isValidNum, IsValidStr, isValidEnum, isValidBody, isValid, isValidEmail, isValidMobile, isValidSize, isValidPrice,isValidGender }
